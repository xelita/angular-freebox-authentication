var app = angular.module("app", ['fbAuthModule']);
...
app.controller("appCtrl", function ($scope, fbAuthService, fbCommonService) {
    ...
    $scope.apiVersion = function () {
        fbCommonService.apiVersion().then(function (response) {
    ...
    $scope.requestAuthorization = function () {
        var tokenRequest = {
            'app_id': 'fr.furiousapps.stopwifi',
            'app_name': 'StopWifi',
            'app_version': '2.0.0',
            'device_name': 'iPhone de Benjamin'
        };

        fbAuthService.requestAuthorization(tokenRequest).then(function (response) {
            $scope.requestAuthorization = response.data;

            var trackingId = response.data.result.track_id;
            ...
            fbAuthService.trackAuthorizationProgressUntil(trackingId, appToken).then(function (response) {
                alert('OK');
    ...
    $scope.session = function () {
        ...
        fbAuthService.login().then(function (response) {
            var challenge = response.data.result.challenge;
            console.log('challenge: ' + challenge);
            console.log('token: ' + $scope.appToken);

            fbAuthService.session('fr.furiousapps.stopwifi', $scope.appToken, challenge).then(function (response) {
                $scope.sessionToken = response.data.result.session_token;
